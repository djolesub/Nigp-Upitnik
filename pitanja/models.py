...
class OrganizacijaOsnovniPodaci(models.Model):
	...
	Adresa = models.CharField(max_length=50, verbose_name='Адреса', help_text="Место, Улица и број: (нпр. Београд, Ресавска 25)")
	Internet_sajt = models.URLField(verbose_name='Интернет сајт', help_text="Линк ка сајту: (нпр. www.rgz.gov.rs)")
	E_mail = models.EmailField(verbose_name='Е-адреса', help_text="нпр. primer@mail.com")
	...
	E_mail_kontakt_osobe = models.EmailField(verbose_name='Е-адреса',help_text="нпр. primer@mail.com")
...
Sekcija2_5_Program_Choices = (
	#Dodato naknadno
	('Не користимо ову врсту апликације','Не користимо ову врсту апликације'),
...
class Pitanje2_5(models.Model):
	...
	Jedinstveni = models.CharField(max_length = 100, null=True, blank=True)
	#Dodatno Za Sva Ostalo 
	Program_ostalo = models.CharField(max_length=100, verbose_name="Program_ostalo", null=True, blank=True)
	Namena_ostalo = models.CharField(max_length=100, verbose_name="Namena_ostalo ", null=True, blank=True)
...
class KadroviInfrastruktura(models.Model):
	...
	Koje_aplikacije_su_u_upotrebi_za_obradu =models.ForeignKey('Pitanje2_5', unique=True, blank = True, on_delete=models.CASCADE) 
	Koji_sistemi_za_upravljanje_bazama_imate = models.ForeignKey('Pitanje2_6',unique=True, blank = True, on_delete=models.CASCADE) 
	...
	Mobilne_gis_aplikacije = models.ForeignKey('Pitanje2_8',unique=True, blank = True, on_delete=models.CASCADE) 
	Jedinstveni = models.CharField(max_length = 100, null=True, blank=True)
...
class Pitanje3_3(models.Model):
	...
	Jedinstveni = models.CharField(max_length = 100, null=True, blank=True)
	Proizvod_ostalo = models.CharField(max_length=100, verbose_name="Proizvod_ostalo", null=True, blank=True)
	Proizvodjac_ostalo = models.CharField(max_length=100, verbose_name="Proizvodjac_ostalo", null=True, blank=True)
	Prostorni_Obuhvat_ostalo = models.CharField(max_length=100, verbose_name="Prostorni_Obuhvat_ostalo", null=True, blank=True)
	def __iter__(self):
		for x in self._meta.get_fields():
			yield x
#####################################################################################################################################################################3
...
class Pitanje3_5(models.Model):
	...
	Proizvodjac_ostalo = models.CharField(max_length=100, verbose_name="Proizvodjac_ostalo", null=True, blank=True)
	Prostorni_obuhvat_ostalo = models.CharField(max_length=100, verbose_name="Prostorni_obuhvat_ostalo", null=True, blank=True)
	Razlog_nedostupnosti_ostalo = models.CharField(max_length=100, verbose_name="Razlog_nedostupnosti_ostalo", null=True, blank=True)
...
class ProstorniPodaci(models.Model):
	Za_koje_od_navedenih_INSPIRE_tema_proizvodite_podatke = models.ForeignKey('Pitanje3_1', unique = True, on_delete=models.CASCADE) 
	Uneti_podatke_o_podacima_koje_vasa_organizacija_proizvodi = models.ForeignKey('Pitanje3_2', unique = True, on_delete=models.CASCADE) 
	Koje_prostorne_podatke_nabavljate_od_drugih = models.ForeignKey('Pitanje3_3', unique=True, on_delete=models.CASCADE)
	Odrediti_vazne_podatke_od_drugih_organizacija_za_redovne_aktivnosti = models.ForeignKey('Pitanje3_4', unique = True, on_delete=models.CASCADE)
	Za_kojim_prostornim_podacima_imate_potrebu_a_koji_nisu_dostupni = models.ForeignKey('Pitanje3_5', unique=True, on_delete=models.CASCADE)
...
class SaradnjaIzmedjuSubjekata(models.Model):
	...
	Ko_su_korisnici_vasih_proizvoda = MultiSelectField(max_length = 300, choices = Selekcija4_3_Korisnici_Proizvoda_Choices)
	...
	Uloga_nacionalnog_geoportala = models.ForeignKey('Pitanje4_7', on_delete=models.CASCADE) 
	Da_li_Podrzavate_razmenu_podataka_preko_portala_e_placanje = models.CharField(max_length = 100, choices = (('Да, подржавамо', 'Да, подржавамо'),('Не, не подржавамо', 'Не, не подржавамо')), null=True, blank=True)
...
Selekcija5_10_Vrsta_Podataka_Choices = (
	...
	('Просторни подаци', 'Просторни подаци'),
...
class StandardizacijaOtvorenihPodataka(models.Model):
	Status_standarda = models.ForeignKey('Pitanje5_1', unique = True, on_delete=models.CASCADE) 
	Standardi_iz_oblasti_geoinformacija = models.ForeignKey("Pitanje5_2", unique=True, on_delete=models.CASCADE)
	...
	Propis_prava_koriscenja = models.ForeignKey("Pitanje5_4", unique = True, on_delete=models.CASCADE)
	...
	Prepreke_za_uspostavljanje_koncepta_gopodataka = models.ForeignKey("Pitanje5_12", unique = True, on_delete=models.CASCADE)
	Vaznost_Prepreka = models.ForeignKey("Pitanje5_13", unique = True, on_delete=models.CASCADE)
	Koliko_se_slazete_sa_tvrdnjama = models.ForeignKey("Pitanje5_14", unique = True, on_delete=models.CASCADE) 
