...
from .models import SveobuhvatniModel, SaradnjaIzmedjuSubjekata, Pitanje2_6
...
def Zavrsna(request):
	return render(request, 'pitanja/Zavrsna.html') 
...
def raw_upitnik(request):	
	if request.POST:
		...
		if(
			...
			sveobuhvatni.save()
			return HttpResponseRedirect(reverse('pitanja:Zavrsna'))
		else:
...
def test(request):
	all = SveobuhvatniModel.objects.all().values()
	#print(all)
	Prva_sekcija_id = all[0]['Prva_sekcija_id']
	Druga_sekcija_id = all[0]['Druga_sekcija_id']
	Treca_sekcija_id = all[0]['Treca_sekcija_id']
	Cetvrta_sekcija_id = all[0]['Cetvrta_sekcija_id']
	Peta_sekcija_id = all[0]['Peta_sekcija_id'] 

	organizacija_osnovni_podaci = OrganizacijaOsnovniPodaci.objects.filter(id = Prva_sekcija_id )
	kadrovi_infrastruktura = KadroviInfrastruktura.objects.filter(id = Druga_sekcija_id) 
	prostorni_podaci = ProstorniPodaci.objects.filter(id = Treca_sekcija_id) 
	saradnja_izmedju_subjekata = SaradnjaIzmedjuSubjekata.objects.filter(id = Cetvrta_sekcija_id)
	standardizacija_otvorenih_podataka = StandardizacijaOtvorenihPodataka.objects.filter(id = Peta_sekcija_id)

	organizacija_osnovni_podaci = organizacija_osnovni_podaci.values()
	kadrovi_infrastruktura = kadrovi_infrastruktura.values()
	sistemi_za_upravljanje_bazama = kadrovi_infrastruktura['Koji_sistemi_za_upravljanje_bazama_imate'] 
	#return HttpResponse(kadrovi_infrastruktura)
	#sistemi_za_upravljanje_bazama = sistemi_za_upravljanje_bazama.values()
	

	return render(request, 'pitanja/moje.html', {
													'organizacija_osnovni_podaci':organizacija_osnovni_podaci,
													'kadrovi_infrastruktura':kadrovi_infrastruktura,
													'sistemi_za_upravljanje_bazama': sistemi_za_upravljanje_bazama
													})   